...
async def copy_media(message, chat_id, caption=None, parse_mode=ParseMode.HTML, protect_content=False):
    try:
        ...
        LOGGER.warning(str(f))
        await sleep(f.value * 1.2)
        return await copy_media(message, chat_id, caption, parse_mode, protect_content)
    except Exception as e:
        LOGGER.error(str(e))
        return str(e)
    return None
...
async def copy_media_group(client, chat_id, send_medias, protect_content=False):
    try:
        await client.send_media_group(chat_id, send_medias, protect_content=protect_content)
    except FloodWait as f:
        LOGGER.warning(str(f))
        await sleep(f.value * 1.2)
        return await copy_media_group(client, chat_id, send_medias, protect_content)
    except Exception as e:
        LOGGER.error(str(e))
        return str(e)
    return None
