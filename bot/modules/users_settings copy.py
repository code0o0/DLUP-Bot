...
async def get_buttons(from_user, edit_type=None, key=None):
    ...
    if edit_type is None:
        ...
        buttons.data_button("Gdrive Tools", "userset {user_id} gdrive")
        ...
        msg = f"{edit_type.capitalize()} Settings:\n"
        if edit_type == "aria":
            for index, key in enumerate(list(user_dict.get("aria2_user", {}).keys()), start=1):
                value = str(user_dict["aria2_user"][key])
                msg += f'{index}. <b>{key}</b> is <u>{value}</u>\n'
                buttons.data_button(index, f"userset {user_id} ariavar {key}")
            buttons.data_button("Default", f"userset {user_id} ariavar default", position="body")
        elif edit_type == "yt-dlp":
            msg = "YT-DLP Options:\n"
            msg += f"YT-DLP Options is <code>{user_dict.get('yt_user', 'None')}</code>\n"
            msg += f"If you want to change, send new options.\n"
            msg += "Format: key:value|key:value|key:value.\n"
            msg += "Example: format:bv*+mergeall[vcodec=none]|nocheckcertificate:True"
            msg += "Timeout: 60s."
        elif edit_type == "upload":
            for index, key in enumerate(list(user_dict.get("upload_user", {}).keys()), start=1):
                value = str(user_dict["upload_user"][key])
                msg += f'{index}. <b>{key}</b> is <u>{value}</u>\n'
                buttons.data_button(index, f"userset {user_id} uploadvar {key}")
            buttons.data_button("Default", f"userset {user_id} uploadvar default", position="body")
        elif edit_type == "leech":
            for index, key in enumerate(list(user_dict.get("leech_user", {}).keys()), start=1):
                value = str(user_dict["leech_user"][key])
                msg += f'{index}. <b>{key}</b> is <u>{value}</u>\n'
                buttons.data_button(index, f"userset {user_id} leechvar {key}")
            buttons.data_button("Default", f"userset {user_id} leechvar default", position="body")
